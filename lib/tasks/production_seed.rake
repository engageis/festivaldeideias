desc "This task is for adding initial production system database"
task :production_seed => :environment do
  ...
  site = Site.create! :name => "Festival de Ideias · Centro Ruth Cardoso", :host => "ramify.me", :port => nil, :auth_gateway => true, :template => site_template, :twitter => "centrorcardoso", :image => File.open("#{Rails.root.to_s}/lib/fixtures/site_logo.png")
